...
from utils import parse_json, gpt_call, read_prompt_from_file, gpt_call_append, get_client, open_source_llm_call, open_source_llm_append
import subprocess
import psutil
import time
...
class InAttack:
    def __init__(self, config: InAttackConfig, starter = 1):
        ...
        #self.target_client = get_client('claude')
        ...
        self.total = 0.0
        self.success = 0.0
        self.call_number = 12 #hyper-parameter
    ...
    def attack_single(self, query_number: int):
            ...
            for sample in topic.get("samples", []):
                ...
                for i in range(len(ids)):
                    ...
                    if success_status == 1 and prune_state == False: #successful case in simulation and not pruned
                        ...
                        call_number +=1
                        if call_number > self.call_number:
                            return output_data
                            ...
                            for _ in range(4): 
                                ...
                                try:
                                    ...
                                    if resp==None:
                                        ...
                                        #resp, dialog_hist = open_source_llm_append('llama', [] if dialog_hist==None else dialog_hist, query) # hyper-parameter optimization *** victom model is open-source llms***
                                    ...
                                    print('_type_',score,_type_)
                                    if score == 5:
                                        is_succeed = True
                                    elif "rejective" in _type_ and "successful" not in _type_:
                                        prompt = self.modify_prompt.format(queries=queries_str, query=query, previous_response='' if len(real_responses)<2 else real_responses[-2],target_obj = org_query)
        ...
        print('\n statistics: ', preattack_file_path,  total_counter , prune_counter) 
    ...
    def action(self, num = -1):
        ...
        with ThreadPoolExecutor(max_workers = 50) as executor:
            json_data['data'] = list(executor.map(self.attack_single, list(range(self.starter, num + self.starter))))
        ...
        print("\nsuccess: ",self.success/self.total)
